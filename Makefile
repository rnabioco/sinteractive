...
# The Rust binary (static musl, so one build runs on every node). The install
# targets take it when it has been built and fall back to the bash script
# with a warning while the rewrite is in progress; `make build` produces it.
MUSL_TARGET := x86_64-unknown-linux-musl
BIN := target/$(MUSL_TARGET)/release/sinteractive
...
build:
	cargo build --release --target $(MUSL_TARGET) -p sint
