...
# The Rust binary. It links against glibc, so build it on the oldest glibc
# it must run on (Rocky 8 on Alpine); one such build runs on every node. The
# install targets take it when it has been built and fall back to the bash
# script with a warning while the rewrite is in progress; `make build`
# produces it. FEATURES adds cargo features (web_server_capability for the
# zellij web client).
BIN := target/release/sinteractive
...
build:
	cargo build --release -p sint --features "$(FEATURES)"

# The man page and bash completions are generated by the binary itself, so
...
# Copy the binary when it exists, else the script (warning), to $(1).
define install_program
	@if [ -x $(BIN) ]; then \
	  install -m 0755 $(BIN) $(1); \
	else \
	  echo "warning: $(BIN) not built (make build); installing the bash script instead" >&2; \
	  install -m 0755 sinteractive $(1); \
	fi
endef
...
install-user:
	mkdir -p $(PREFIX)
	$(call install_program,$(PREFIX)/sinteractive)
	mkdir -p $(MANDIR)
...
install-system:
	$(call install_program,/usr/local/bin/sinteractive)
...
nodes-check:
	@for n in $(NODES); do \
	  ...
	  ssh -o BatchMode=yes -o ConnectTimeout=5 $(SSH_USER)@$$n ' \
	    v=$$(/usr/local/bin/sinteractive --version 2>/dev/null | sed -n "s/^sinteractive //p" | head -1); \
