//! zellij, compiled in.
//!
//! This module is zellij's own `src/main.rs` and `src/commands.rs` (v0.45.1,
//! MIT — see `LICENSE-zellij.md` beside this file) with one change: `main`
//! became [`run`], which takes argv instead of reading the process's. It
//! makes `sinteractive` the zellij client *and* server: the client library
//! spawns the server by re-executing `current_exe()` with `--server PATH`,
//! which `main.rs` routes here before clap sees it, and every internal
//! zellij invocation (`list-sessions`, `action …`, `attach`, `kill-session`)
//! goes through `sinteractive zellij …`. `ZELLIJ_SOCKET_DIR` and friends are
...
/// Run zellij with `argv` (argv[0] is the program name, e.g. `zellij`).
/// Never returns: zellij's entry points call `process::exit` themselves,
/// so this ends the process too.
pub fn run(argv: Vec<String>) -> ! {
    configure_logger();
    create_config_and_cache_folders();
    let opts = CliArgs::parse_from(argv); // sinteractive: was CliArgs::parse()
    ...
    std::process::exit(0); // sinteractive: `run` never returns
}
