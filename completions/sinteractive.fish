...
complete -c sinteractive -n "__fish_sinteractive_needs_command" -f -a "quota" -d 'Storage quota (Bodhi daemons; wraps curc-quota on Alpine)'
...
complete -c sinteractive -n "__fish_sinteractive_using_subcommand help; and not __fish_seen_subcommand_from launch attach ensure status refresh list cancel queue monitor snapshot events peek send agent-context quota hook statusline mcp install-claude doctor completions man schema __job __attach __popup help" -f -a "quota" -d 'Storage quota (Bodhi daemons; wraps curc-quota on Alpine)'
